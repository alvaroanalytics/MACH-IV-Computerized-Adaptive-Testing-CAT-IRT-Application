...
# Description: Production script as an open source. GITHUB REPOSITORIES.
#              Includes IRT logic, CSS, and dynamic reporting.
# LinkedIn: https://www.linkedin.com/in/alvarogonzalezsanchezpsicologia/
...
# Inverse items (1-5) to (5-1)
items_inversos <- c(3, 4, 6, 7, 9, 10, 11, 14, 16, 17)

# Defining item clusters here helps us calculate and display specific sub-dimension means in the  final report, giving the user more granular feedback.
...
final_fun_pro <- function(person) {
  ...
  # We extract the final Standard Error (SEM) from the history
  ...
  # We calculate the approximate Reliability (r = 1 - SEM^2)
  reliability_est <- max(0, 1 - (final_sem^2))
  
  # 2. DATA LOGGING 
  # This block saves the psychometric properties of this session to a CSV file. "append = TRUE" ensures we add a new row for each participant without deleting previous ones.
...
# We increase the minimum items to 10. This gives the MAP algorithm enough data points to  confidently estimate the trait before stopping, ensuring higher reliability.
...
# Finally, we launch the test. Using 'MAP' (Maximum A Posteriori) is crucial here because it allows the score to move to extremes (high/low Machiavellianism) faster than the default method.
...
# ACTUALIZATION IN PROGRESS... Striving for better, optimal & more accurate performance.
# Just a MSc student doing his best against the machine & Psychometrics :)

# P.S.: Found a bug, error, fail or have a suggestion? Contact me without hesitation. Cheers!!
